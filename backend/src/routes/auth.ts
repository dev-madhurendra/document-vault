...
import { sendOtpEmail } from "../utils/mailer"; // Ensure you have your mailer utility set up
...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post<{ Body: SignupBody }>("/api/auth/signup", async (request, reply) => {
    ...
    await sendOtpEmail(formattedEmail, otp);
    return reply.code(201).send({ message: "Signup successful. Please check your email for the OTP code." });
  });

  // 2. VERIFY OTP: Validates code and responds with JWT token
  app.post<{ Body: VerifyOtpBody }>("/api/auth/verify-otp", async (request, reply) => {
  ...
  app.post<{ Body: ResendOtpBody }>("/api/auth/resend-otp", async (request, reply) => {
